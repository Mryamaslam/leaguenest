(function(){
  ...
  // cart helpers
  function getCart(){try{return JSON.parse(localStorage.getItem("leaguenest_cart")||"[]")}catch(e){return[]}}
  function setCart(c){localStorage.setItem("leaguenest_cart",JSON.stringify(c));updateCartCount()}
  ...
  if(checkoutForm)checkoutForm.addEventListener("submit",function(e){
    e.preventDefault();
    localStorage.removeItem("leaguenest_cart");
    updateCartCount();
